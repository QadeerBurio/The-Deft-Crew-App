...
(packets P19 and P20).
**Loyalty points, levels, referrals, partners (ambassadors and influencers), brand campaigns and Founder Circle entry are specified in `TDC_CREW.md`,
...
> **Superseded by `TDC_CREW.md` §2–3** (points, levels, referral verification, values).
...
| ~~P15 Rewards / points spend~~ → see TDC_CREW §10 (C1–C4) | both | `routes` rewards endpoints, app `RewardsScreen` | `components/Points.js` (balance + entry) | concurrent redeems can't overdraw; ledger sums to balance |
...
| **P20a/b Founder Circle** (entry rules per TDC_CREW §3) | both | see LAUNCH_AND_FOUNDER_CIRCLE.md §B.8 | `Card.js`, `Points.js`, `FounderCircleScreen.js`, `ProfileScreen.js` | ≤ 50 seats; three card editions |
