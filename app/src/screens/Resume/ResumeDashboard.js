...
const ResumeDashboard = ({ navigation }) => {
  ...
  const insightSlide = useRef([...Array(2)].map(() => new Animated.Value(30))).current;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#f9c349" colors={["#f9c349"]} />
        }
      >
        {/* Header */}
        ...
        {/* Quick Actions - Perfect Row Layout */}
          ...
          {actionButtons.map((action, index) => (
            <Animated.View
              ...
              style={{
                ...
                transform: [{ 
                  translateY: actionStagger[index].interpolate({
                    inputRange: [0, 1],
                    outputRange: [30, 0],
                  })
                }, {
                  scale: actionStagger[index].interpolate({
                    inputRange: [0, 1],
                    outputRange: [0.8, 1],
                  })
                }],
            ...
            {marketInsights.slice(0, 2).map((insight, index) => (
              <Animated.View
                ...
                style={{
                  ...
                  opacity: fadeAnim,
                  transform: [{ 
                    translateY: insightSlide[index].interpolate({
                      inputRange: [0, 30],
                      outputRange: [30, 0],
                    })
                  }],
              ...
              >
                <InsightCard
                  ...
                  match={insight.match}
                  trend={insight.trend}
                  icon={index === 0 ? "trending-up" : "cpu"}
                  color={index === 0 ? "#f9c349" : "#1a1a1a"}
                />
              </Animated.View>
            ))}
          </View>
          ...
          >
            <View style={[styles.jobIconBg, { backgroundColor: rec.priority === 'high' ? '#1a1a1a' : '#f9c349' }]}>
              <MaterialCommunityIcons
                ...
                size={24}
                color={rec.priority === 'high' ? '#f9c349' : '#1a1a1a'}
              />
            </View>
  ...
  >
    <View style={[styles.actionIconCircle, { backgroundColor: disabled ? '#f5f5f5' : '#1a1a1a' }]}>
      {loading ? (
        ...
        <Ionicons name={icon} size={20} color={disabled ? '#ccc' : '#f9c349'} />
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  // Action Row - Perfect Horizontal Layout
  actionRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "flex-start",
